...
moco_transform_breakhis = transforms.Compose([
            ...
            transforms.Normalize(
            mean=[0.7861, 0.6264, 0.7639], std=[0.1060, 0.1383, 0.0905]) # BREAKHIS DATASET
            ])
...
if __name__ == '__main__':
    ...
    for cycle in range(CYCLES):
        ...
        if cycle < CYCLES-1:
			...
			######### PUT ACTIVE SELECTION STUFF HERE ########
            ...
            while (len(list0) < NUM_CLASSES and i < 3):
                for splitnum, phase in enumerate(splits):
                    for item in phase:             # arg is the query list                         
                        ...
                        if (p_label not in list0 and item not in selected_samples) or (i == 2 and len(list0) < ADDENDUM):
                            selected_samples.append(item)
