...
#####Code here should be changed.#####
DATA_LEN = 768*768*3 # 224*224*3  
CHANNEL_LEN = 768*768 #224*224
SHAPE = 768
#####################################
...
def read_data(filename, data_path, shape=None, color='RGB'):
    ...
    if os.path.isdir(filename): # filename here will be cifar10Dataset-master copy/data/images.lst
        ...
        for ln in lines:
            fname, lab = ln.split(' ')
            path_1=os.path.join(data_path, fname)
            im = imread(path_1, shape=s, color='RGB')
            '''
